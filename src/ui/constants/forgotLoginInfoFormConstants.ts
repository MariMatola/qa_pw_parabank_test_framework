export interface ForgotLoginInfoFormField {
    name: string;
    locator: string;
    emptyErrorMessage: string;
}

export const ForgotLoginInfoFormFields: ForgotLoginInfoFormField[] = [
    {
        name: 'First Name',
        locator: '#firstName',
        emptyErrorMessage: 'First name is required',
    },
    {
        name: 'Last Name',
        locator: '#lastName',
        emptyErrorMessage: 'Last name is required',
    },
    {
        name: 'Address',
        locator: '[id="address.street"]',
        emptyErrorMessage: 'Address is required',
    },
    {
        name: 'City',
        locator: '[id="address.city"]',
        emptyErrorMessage: 'City is required',
    },
    {
        name: 'State',
        locator: '[id="address.state"]',
        emptyErrorMessage: 'State is required',
    },
    {
        name: 'Zip Code',
        locator: '[id="address.zipCode"]',
        emptyErrorMessage: 'Zip Code is required',
    },
    {
        name: 'SSN',
        locator: '#ssn',
        emptyErrorMessage: 'Social Security Number is required',
    },
];
